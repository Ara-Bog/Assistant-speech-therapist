...
export default class TimetableWrap extends Component  {
    ...
    getData(selectDay) {
        db.transaction((tx) => {
            tx.executeSql(
            "SELECT tt.id, tt.time, gr.name, dg.name as Diagnos, ct.name as Categori, 'g' as Type FROM Timetable as tt, Diagnosis as dg, Categories as ct INNER JOIN Groups as gr On gr.id = tt.client_id and gr.diagnos_id = dg.id WHERE tt.Type = 'g' and gr.categori_id = ct.id and tt.day = ?"
            +" UNION "
            +"SELECT tt.id, tt.time, st.name || ' ' || st.surname as name, dg.name as Diagnos, ct.name as Categori, 's' as Type FROM Timetable as tt, Diagnosis as dg, Categories as ct INNER JOIN students as st On st.id = tt.client_id and st.diagnos_id = dg.id WHERE tt.Type = 's' and st.categori_id = ct.id and tt.day = ?", 
            [selectDay, selectDay], 
...
// dobavit group_page
    NavigatePage(ID, Type){
        let way = ''
        {Type == 's'? way = 'Student': way = 'Student'}
        ...
        return (
            <View style={Styles.container}>
                ...
                <View style={Styles.rowSwitch}>
                    ...
                    />
                    <Text style={{fontSize: 14, fontFamily:'sf_medium', color: this.state.editing? '#554AF0':'#04021D', marginTop: -4, marginLeft: 15}}>Редактировать</Text>
                </View>
                            ...
                            <View style={Styles.cardDaysBtns}>
                                {this.state.editing ? 
                                    ...
                                    onPress={() => this.getCurrentClient(item.ID) }>
                                        <Feather name="edit-3" size={16} color="#554AF0" />
                                    ...
                                    onPress={() => this.NavigatePage(item.ID, item.Type)}>
